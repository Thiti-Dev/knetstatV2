...
static int totalShow = 1;
static int inspectOnly = 0;
static int portOnly = -1;
module_param(inspectOnly, int, S_IRUGO);
module_param(portOnly, int, S_IRUGO);
...
static int tcp_seq_show(struct seq_file *seq, void *v) {
	if (v == SEQ_START_TOKEN) {
		totalShow=1;
		seq_printf(seq, "Count Recv-Q Send-Q Local Address           Foreign Address         Stat Diag Options\n\n");
	}
	else if (v == NULL){
		seq_printf(seq, "END \n");
	} else {
		...
		int tokenCheck = 0;
		if(inspectOnly == 0){ tokenCheck = 1;}
		else if(inspectOnly == 1 && state == TCP_LISTEN){ tokenCheck = 1;}
		else if(inspectOnly == 2 && state == TCP_ESTABLISHED){ tokenCheck = 1;}
		else if(inspectOnly == 3 && state == TCP_TIME_WAIT){ tokenCheck = 1;}
		else if(inspectOnly == 4 && state == TCP_FIN_WAIT1){ tokenCheck = 1;}
		else if(inspectOnly == 5 && state == TCP_FIN_WAIT2){ tokenCheck = 1;}
		else if(inspectOnly == 6 && state == TCP_SYN_SENT){ tokenCheck = 1;}
		else if(inspectOnly == 7 && state == TCP_SYN_RECV){ tokenCheck = 1;}
		else if(inspectOnly == 8 && state == TCP_NEW_SYN_RECV){ tokenCheck = 1;}
		else if(inspectOnly == 9 && state == TCP_CLOSE){ tokenCheck = 1;}
		else if(inspectOnly == 10 && state == TCP_CLOSE_WAIT){ tokenCheck = 1;}
		else if(inspectOnly == 11 && state == TCP_CLOSING){ tokenCheck = 1;}
		else if(inspectOnly == 12 && state == TCP_LAST_ACK){ tokenCheck = 1;}
		
		if ( portOnly != -1 && srcp != portOnly && destp != portOnly ){ tokenCheck=0;}
		
		if(tokenCheck == 1){
			seq_printf(seq, "%5d %6d %6d ", totalShow,rx_queue, tx_queue);
